...
select * from student_course_view
 ...
 update students set stuname='roger federer' where stid=1;
 update students set stuname='mirka federer' where stid=2;
 update students set stuname='paluvenchekova' where stid=3;
 update students set stuname='john arogiasamy' where stid=4;
 update students set stuname='djokovic' where stid=5;
 update students set stuname='stefanie graf' where stid=6;
 show tables;
 
 create table students_history(stid int, stuname varchar(100), deleted_time timestamp);
...
for each row
begin
insert into students_history(stid, stuname, deleted_time)
values(old.stid, old.stuname, now());
end$$
delimiter ;

delete from students where stid=4;

select * from students_history;

-- deleted records are logged automatically in students_history

create user 'assistant_corruptist'@'%' 
identified by 'people123';

grant select on college.students to 'assistant_corruptist'@'%';

revoke select on college.students from 'assistant_corruptist'@'%';

-- User privileges are updated accordingly.

start transaction;
update students set stuname='mirnalini' where stid=4;
savepoint sp1;
rollback to sp1;
commit;
